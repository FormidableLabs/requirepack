...
/**
 * Template output for empty match.
 */
Manifest.prototype.EMPTY_TEMPLATE = "function () {}";
...
Manifest.prototype.mapNameToNum = function (name) {
  ...
  // Remove empties
  if (_.includes(this._opts.empty, name)) {
    return this.EMPTY_TEMPLATE;
  }

  // Perform all transforms first.
  ...
  // TODO HERE -- custom-alias/templates/message.hbs
  // - Probably going to need full webpack configuration and `alias` application
  // - See if can do with `webpack` import for real normalization / resolution?
  console.log("TODO HERE", name);
