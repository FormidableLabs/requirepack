 ...
 * @param {String} opts.requirejsRoot           Root of RequireJS build (inferred of config path)
...
var Compiler = module.exports = function (opts) {
  ...
  this._manifest = new Manifest({
    ...
    requirejsRoot: this._opts.requirejsRoot,
    requirejsConfig: this._opts.requirejsConfig
  });
};
...
Compiler.prototype._initOptions = function (opts) {
  ...
  if (_.isString(opts.requirejsConfig)) {
    // Infer unset RequireJS root path off config path.
    if (!opts.requirejsRoot) {
      opts.requirejsRoot = path.dirname(opts.requirejsConfig);
    }
