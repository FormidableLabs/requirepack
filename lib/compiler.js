 ...
 * @returns {Number}        Webpack lookup number or `null`
 */
Compiler.prototype._mapNameToNum = function (name) {
  return name ? 1 : 2;/*TODO*/
};
...
Compiler.prototype.template = function (names) {
  var manifest = this._opts.webpackManifest;
  ...
  return [].concat(
    ...
    _.map(names, function (name) {
      var num = this._mapNameToNum(name);
