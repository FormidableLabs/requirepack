 ...
 * @param {Array<Object>} opts.transforms       Objects of `test` regex and `transform` fns
 * @param {Object} opts.alias                   Name aliases.
 * @param {Object} opts.templates               Direct strings to insert in interop
...
Compiler.prototype.template = function (names) {
  ...
  return [].concat(
    ...
    _.map(names, function (name) {
      ...
      // Default to resolved string.
      var resolution = num.toString();
      if (_.isNumber(num)) {
        // A Webpack-RequireJS mapping by number.
        resolution = "translate(" + num + ")";
      }

      return "  def(\"" + name + "\", " + resolution + ");";
    }),
    [
      "}());"
    ]).join("\n");
